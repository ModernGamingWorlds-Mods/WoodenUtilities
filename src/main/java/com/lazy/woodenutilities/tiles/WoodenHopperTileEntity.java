...
import com.lazy.woodenutilities.block.WoodenHopperBlock;
...
public class WoodenHopperTileEntity extends LockableLootTileEntity implements IHopper, ITickableTileEntity {
    ...
    public WoodenHopperTileEntity() {
        super(ModTiles.WOODEN_HOPPER_TILE.get());
    }
    ...
    private boolean updateHopper(Supplier<Boolean> p_200109_1_) {
        if (this.world != null && !this.world.isRemote) {
            if (!this.isOnTransferCooldown() && this.getBlockState().get(WoodenHopperBlock.ENABLED)) {
                boolean flag = false;
    ...
    private boolean transferItemsOut() {
        ...
        if (iinventory == null) {
            ...
            Direction direction = this.getBlockState().get(WoodenHopperBlock.FACING).getOpposite();
    ...
    private IInventory getInventoryForHopperTransfer() {
        Direction direction = this.getBlockState().get(WoodenHopperBlock.FACING);
